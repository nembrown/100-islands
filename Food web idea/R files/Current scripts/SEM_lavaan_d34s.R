...
sem_variables_names_sn<-c("node", "unq_tran","unq_isl", "log_fish_biomass_bym3_mean", "log_bycatch_biomass_bym3_mean",
                           ...
                           "slope_isl", "CHM_mean_height", "habitat_het", "NDVI_mean", "d15n", "n", "s")



master_transec_sem_subset_sn<-master_transect[, colnames(master_transect) %in% sem_variables_names_sn]
master_transec_sem_subset_sn$unq_tran<-factor(master_transec_sem_subset_sn$unq_tran, ordered=TRUE)
master_transec_sem_subset_sn$unq_isl<-factor(master_transec_sem_subset_sn$unq_isl, ordered=TRUE)
master_transec_sem_subset_sn$node<-factor(master_transec_sem_subset_sn$node)

master_transec_sem_subset_sn_centered <- stdize(master_transec_sem_subset_sn, 
...
head(master_transec_sem_subset_sn_centered)

#see structure of the data, which variables have missing data
summary(master_transec_sem_subset_sn_centered)
md.pattern(master_transec_sem_subset_sn_centered)
...
S34_N15_model_simple_centered_alt<-'
        ...
        marine_chem ~ w1*c.log_site_mean_by_tran + h1*c.distance_to_midden + pres_marine_invert + pres_fish + c.slope_degrees  + c.WAVE_EXPOSURE + pres_otter + ravens + eagles + elevation_max + c.slope_isl  + c.log_Bog_area + c.CHM_mean_height

        c.CHM_mean_height ~ c.NDVI_mean + c.log_Area + elevation_max + c.PA_norml + c.slope_isl + c.WAVE_EXPOSURE + c.log_Bog_area 
        
        #latent marine chem
        marine_chem =~ c.d34s + c.d15n + c.s + c.n   '
...
fml.sn <- list( c.log_fish_biomass_bym3_mean + c.log_bycatch_biomass_bym3_mean +
                   ...
                   pres_marine_invert + pres_fish + c.log_MEAN_rockarea2000 + c.d15n + c.n + c.s ~ 1 + (1|unq_isl) ,                                                 # Level 1
...
imp.sn <- jomoImpute(master_transec_sem_subset_sn_centered, formula=fml.sn, n.burn=500, n.iter=1000, m=20)
summary(imp.sn)
write.mitml(imp.sn, "C:Food web idea//Data by person//Norah.data/imp.sn")

#load it 
imp.sn <- read.mitml("C:Food web idea//Data by person//Norah.data/imp.sn")

#problems the model sees: 
plot(imp.sn, trace="all", print="beta2", pos=c(1,7))
plot(imp.sn, trace="all", print="beta", pos=c(1,1))
plot(imp.sn, trace="all", print="psi", pos=c(17,6))
plot(imp.sn, trace="all", print="sigma", pos=c(4,2))

implist_sn <- mitmlComplete(imp.sn, "all")
#get it to talk to survey.design
imputed_mitml_sn <- imputationList(implist_sn)


# Option 2 Amelia ---------------------------------------------------------
set.seed(12345)
HS.amelia_sn <- amelia(master_transec_sem_subset_sn_centered, m = 20, idvars = c("unq_tran", "unq_isl", "node"), p2s = FALSE)
imps_sn <- HS.amelia_sn$imputations
imps_amelia_sn<-imputationList(imps_sn)

# Running model and adding survey design ----------------------------------

#run basic empty model then update with survey design
lavaan_fit_model_sn<-sem(S34_N15_model_simple_centered_alt, data=master_transec_sem_subset_sn_centered)
summary(lavaan_fit_model_sn)

varTable(lavaan_fit_model_sn)

# Survey.design with mitml and amelia - mice wasn't working 
design_imp_amelia_sn<-svydesign(ids=~unq_isl, strata=~node, data=imps_amelia_sn)
design_imp_mitml_sn<-svydesign(ids=~unq_isl, strata=~node, data=imputed_mitml_sn)

fit.adj.amelia.sn<-lavaan.survey(lavaan.fit=lavaan_fit_model_sn, survey.design = design_imp_amelia_sn, estimator="MLMVS")
summary(fit.adj.amelia.sn, standardized=T)

fit.adj.mitml.sn<-lavaan.survey(lavaan.fit=lavaan_fit_model_sn, survey.design = design_imp_mitml_sn, estimator="MLMVS")
summary(fit.adj.mitml.sn, standardized=T)
