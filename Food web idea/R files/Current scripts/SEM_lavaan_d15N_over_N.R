...
sem_variables_names_d15N_over_N <-c("node", "unq_tran","unq_isl", "log_fish_biomass_bym3_mean", "log_bycatch_biomass_bym3_mean",
                         "SLOPE_degrees", "log_Area", "WAVE_EXPOSURE", "beachy_substrate", "slope_degrees",
                         "ravens",  "d15N_over_N", "distance_to_midden",
...
master_transec_sem_subset_d15N_over_N<-master_transect[, colnames(master_transect) %in% sem_variables_names_d15N_over_N]
master_transec_sem_subset_d15N_over_N$unq_tran<-factor(master_transec_sem_subset_s$unq_tran, ordered=TRUE)
master_transec_sem_subset_d15N_over_N$unq_isl<-factor(master_transec_sem_subset_s$unq_isl, ordered=TRUE)
master_transec_sem_subset_d15N_over_N$node<-factor(master_transec_sem_subset_s$node)

master_transec_sem_subset_d15N_over_N_centered <- stdize(master_transec_sem_subset_d15N_over_N , 
...
head(master_transec_sem_subset_d15N_over_N_centered)

#see structure of the data, which variables have missing data
summary(master_transec_sem_subset_d15N_over_N_centered)
md.pattern(master_transec_sem_subset_d15N_over_N_centered)
...
model_simple_centered_alt_d15N_over_N <-'
        ...
        c.d15N_over_N ~ w1*c.log_site_mean_by_tran + h1*c.distance_to_midden + pres_marine_invert + pres_fish + c.slope_degrees  + c.WAVE_EXPOSURE + pres_otter + ravens + eagles + elevation_max + c.slope_isl  + c.log_Bog_area + c.CHM_mean_height
...
fml.d15N_over_N  <- list( c.log_fish_biomass_bym3_mean + c.log_bycatch_biomass_bym3_mean +
                 c.SLOPE_degrees  + c.WAVE_EXPOSURE + beachy_substrate + c.slope_degrees + c.log_site_mean_by_tran  + 
                 c.d15N_over_N + c.distance_to_midden + c.log_MEAN_kparea2k + c.log_MEAN_egarea2k + pres_otter +
...
imp.d15N_over_N <- jomoImpute(master_transec_sem_subset_d15N_over_N_centered, formula=fml.d15N_over_N, n.burn=500, n.iter=1000, m=20)
summary(imp.d15N_over_N )
write.mitml(imp.d15N_over_N , "C:Food web idea//Data by person//Norah.data/imp.d15N_over_N ")

#load it 
imp.d15N_over_N  <- read.mitml("C:Food web idea//Data by person//Norah.data/imp.d15N_over_N ")

#problems the model sees: 
plot(imp.d15N_over_N , trace="all", print="beta2", pos=c(1,7))
plot(imp.d15N_over_N , trace="all", print="beta", pos=c(1,1))
plot(imp.d15N_over_N , trace="all", print="psi", pos=c(17,6))
plot(imp.d15N_over_N , trace="all", print="sigma", pos=c(4,2))

implist_d15N_over_N  <- mitmlComplete(imp.d15N_over_N , "all")
#get it to talk to survey.design
imputed_mitml_d15N_over_N  <- imputationList(implist_d15N_over_N )


# Option 2 Amelia ---------------------------------------------------------
set.seed(12345)
HS.amelia_d15N_over_N  <- amelia(master_transec_sem_subset_d15N_over_N_centered, m = 20, idvars = c("unq_tran", "unq_isl", "node"), p2s = FALSE)
imps_d15N_over_N  <- HS.amelia_d15N_over_N$imputations
imps_amelia_d15N_over_N <-imputationList(imps_d15N_over_N )

# Running model and adding survey design ----------------------------------

#run basic empty model then update with survey design
lavaan_fit_model_d15N_over_N <-sem(model_simple_centered_alt_d15N_over_N)
summary(lavaan_fit_model_d15N_over_N)

varTable(lavaan_fit_model_d15N_over_N)

# Survey.design with mitml and amelia - mice wasn't working 
design_imp_amelia_d15N_over_N<-svydesign(ids=~unq_isl, strata=~node, data=imps_amelia_d15N_over_N)
design_imp_mitml_d15N_over_N<-svydesign(ids=~unq_isl, strata=~node, data=imputed_mitml_d15N_over_N)

fit.adj.amelia.d15N_over_N <-lavaan.survey(lavaan.fit=lavaan_fit_model_d15N_over_N, survey.design = design_imp_amelia_d15N_over_N, estimator="MLMVS")
summary(fit.adj.amelia.d15N_over_N , standardized=T)

fit.adj.mitml.d15N_over_N <-lavaan.survey(lavaan.fit=lavaan_fit_model_d15N_over_N, survey.design = design_imp_mitml_d15N_over_N, estimator="MLMVS")
summary(fit.adj.mitml.d15N_over_N , standardized=T)
...
#semplot
grps.d15N_over_N <-list(Algae=c("c.log_MEAN_rockarea2000","c.log_site_mean_by_tran", "c.log_MEAN_kparea2k", "c.log_MEAN_egarea2k" ),
...
semPaths(fit.adj.mitml.d15N_over_N , what="std",  layout=lay_names_d15N_over_N , intercepts=FALSE, residuals=FALSE,
         groups=grps.d15N_over_N , exoVar = FALSE,  color=colour_group, esize=2, nodeLabels = nodelab_d15N_over_N , legend=FALSE)

semPaths(fit.adj.mitml.d15N_over_N , what="path",  layout=lay_names_d15N_over_N , intercepts=FALSE, residuals=FALSE,
         groups=grps.d15N_over_N , exoVar = FALSE, color=colour_group, nodeLabels = nodelab_d15N_over_N , legend=FALSE)



semPlot::semPaths(fit.adj.mitml.d15N_over_N , "path",fade = F, residuals = F, intercepts=FALSE, label.cex=2, nCharNodes = 0, nodeLabels = 1:25)
semPlot::semPaths(fit.adj.mitml.d15N_over_N , "path",fade = F, residuals = F, intercepts=FALSE, label.cex=2, nCharNodes = 0)

AIC(fit.adj.mitml.d15N_over_N )
